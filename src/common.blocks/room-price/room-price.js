class RoomPrice {
    ...
    getPrice(roomPrice){
        let priceText  = roomPrice.querySelector(".room-price__price-sum").textContent;
    ...
    handleDatesModifed(){
        let days =(this.$datePikerObj.selectedDates[1]-this.$datePikerObj.selectedDates[0])/86400000;
        this.priceForRoomDiscriptionContainer.textContent = `${this.getNumWithSpace(this.pricePerDay)}₽ x ${days} суток`;
        let sum = this.pricePerDay * days;
    ...
    handleCalculateModifed(){
        let roomPrice = this.convertToNum(this.priceForRoomPriceContainer.textContent);
        let servicePrice = this.convertToNum(this.priceForServiceContainer.textContent);
        let addServicePrice = this.convertToNum(this.priceForAddServiceContainer.textContent);
        let sum = roomPrice + servicePrice + addServicePrice;        
